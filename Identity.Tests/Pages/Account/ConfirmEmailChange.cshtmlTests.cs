...
namespace Identity.Tests.Pages.Account
{
    ...
    public class ConfirmEmailChangeModelTests
    {
        ...
        public void Constructor_NullParameters_ThrowsIfValidated()
        {
            ...
            Type targetType = typeof(ConfirmEmailChangeModel);
            // Act & Assert
            // We accept either:
        ...
        public async Task OnGetAsync_NullParameters_RedirectsToIndex(string? userId, string? email, string? code)
        {
            ...
            var signInManagerMock = new Mock<SignInManager<IdentityUser<Guid>>>(userManagerMock.Object, Mock.Of<Microsoft.AspNetCore.Http.IHttpContextAccessor>(), Mock.Of<IUserClaimsPrincipalFactory<IdentityUser<Guid>>>(), null, null, null, null);
            var model = new ConfirmEmailChangeModel(userManagerMock.Object, signInManagerMock.Object);
            // Act
            IActionResult result = await model.OnGetAsync(userId, email, code);
            // Assert
        ...
        public async Task OnGetAsync_UserNotFound_ReturnsNotFound()
        {
            ...
            var model = new ConfirmEmailChangeModel(userManagerMock.Object, signInManagerMock.Object);
            // Act
            IActionResult result = await model.OnGetAsync(userId, email, encoded);
            // Assert
        ...
        /// <summary>
        /// Tests that various non-null email inputs including empty, whitespace, very long and special-character strings
        /// are accepted by the handler flow (dependent on UserManager behaviors).
        /// Input conditions: user exists; ChangeEmailAsync and SetUserNameAsync return Success.
        /// Expected result: PageResult and StatusMessage success for each email variant.
        /// </summary>
        [Theory]
        [InlineData("")]
        [InlineData("   ")]
        [InlineData("user+special@exämple.com")]
        public async Task OnGetAsync_VariousEmailInputs_ProceedAndReturnSuccess(string email)
        {
            // Arrange
