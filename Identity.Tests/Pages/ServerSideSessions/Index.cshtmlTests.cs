...
namespace Identity.Tests.Pages.ServerSideSessions;

using Duende.IdentityServer.Services;
using Identity.Pages.ServerSideSessions;
...
/// <summary>Unit tests for <see cref="Identity.Pages.ServerSideSessions.IndexModel"/>.</summary>
[Trait("Category", "Unit")]
public class ServerSideSessionsIndexModelTests
{
    /// <summary>
    /// Verifies that the IndexModel constructor does not throw when the optional service is null.
    ...
    /// <summary>
    /// Verifies that the IndexModel default constructor (no arguments) does not throw.
    ...
    /// Inputs: sessionManagement = null.
    /// Expected: result is RedirectToPageResult pointing to "/ServerSideSessions/Index".
    /// </summary>
    /// <returns>A <see cref="Task"/> representing the asynchronous unit test.</returns>
    [Fact]
    public async Task OnPostAsync_NullService_RedirectsToPage()
    {
        ...
        Assert.Equal("/ServerSideSessions/Index", redirect.PageName);
    }
    ...
    private static IndexModel CreateModel(ISessionManagementService? sessionManagement)
    {
        var model = new IndexModel(sessionManagement);
