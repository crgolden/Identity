...
namespace Identity.Areas.Identity.Pages.Account.Manage
{
    public class PasskeysModel : PageModel
    {
        ...
        private async Task<IActionResult> DeletePasskey([NotNull] IdentityUser<Guid> user, byte[] credentialId)
        {
            ...
            if (!result.Succeeded)
            {
                ...
                throw new InvalidOperationException(
                    $"Unexpected error occurred removing passkey for user with ID '{userId}'.");
            }
        ...
        public async Task<IActionResult> OnPostAddPasskeyAsync()
        {
            ...
            return RedirectToPage("./RenamePasskey",
                new { id = Base64Url.EncodeToString(attestationResult.Passkey.CredentialId) });
        }
    }
}
